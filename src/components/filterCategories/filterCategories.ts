			...
			this.shadowRoot.innerHTML = `
				...
				<section id="cards">
					${moviesData
						.map(
							(movie) => `
						<movie-card image="${movie.image}" uid="${movie.id}" categories="${movie.categories}" utitle="${movie.title}" director="${movie.director}" release_date="${movie.release_date}" cast="${movie.cast}" crew="${movie.crew}" image_sec="${movie.image_sec}"></movie-card>
					`
						)
						.join('')}
				</section>
